            ...
            for(int i=1;i<=k;i++) {
                for(int j=1;j<prices.length;j++) {
                    ...
                    for(int kk=0;kk<j;kk++) {
                        if(maxMax < dp[i-1][kk] + (prices[j] - prices[kk])) {
                            maxMax = dp[i-1][kk] + (prices[j] - prices[kk]);
                        }
                    }
                    dp[i][j] = Math.max(maxMax, dp[i][j-1]);
                }
            }
